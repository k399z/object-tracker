...
int main(){
        ...
        while(true){
                ...
                // The function will now also draw the detected corners onto 'frame'
                Rect bbox = detectChessboardBBox(frame);

                // Draw bounding box if valid
                string status;
                if (bbox.width > 0 && bbox.height > 0) {
                    rectangle(frame, bbox, Scalar(0, 255, 0), 2); // Green bounding box
                ...
                // This frame now includes the drawn corners and the bounding box
